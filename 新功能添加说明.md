...
#### 功能描述
v3.5.4 版本重写了 `[auto_selfie]` 配置部分，添加详细中文注释，让用户配置更加清晰易懂。
...
| `auto_selfie.character_name` | str | `"麦麦"` | 角色名称 |
| `auto_selfie.character_persona` | str | `""` | 角色人设 |
...
#### 功能描述
v3.5.3 版本为 Smart 模式新增了间隔补充发送功能，让自拍时间更加随机自然，增强\"真人感\"体验。
...
### 0.2 🧠 v3.5.2 - Smart智能日程模式（重大更新）

#### 功能描述
v3.5.2 版本是定时自拍功能的革命性升级，引入了 **Smart 智能日程模式**。通过 LLM 动态生成每天的完整日程，自拍动作完全由场景决定，不再依赖固定的动作库。这让 Bot 的自拍行为更加自然、真实、拟人化。
...
│   ├── schedule_models.py           # v3.5.2 新增：日程数据模型
│   ├── schedule_generator.py        # v3.5.2 新增：LLM日程生成器
│   ├── scene_action_generator.py    # v3.5.2 新增：场景动作生成器
│   ├── cache_manager.py
...
- v3.5.2：schedule_mode 默认值改为 smart，新增日程生成相关配置项
...
### schedule_models.py (v3.5.2 新增)
- ScheduleEntry 数据类：日程条目，包含20+字段
- DailySchedule 数据类：每日日程容器
- 支持时间点查询和下一个时间点计算

### schedule_generator.py (v3.5.2 新增)
- LLM日程生成器
- 每日凌晨自动生成当日日程
- 支持自定义日程数量范围

### scene_action_generator.py (v3.5.2 新增)
- 场景动作生成器
- 根据活动类型选择合适动作
- 不依赖固定的hand_actions库
