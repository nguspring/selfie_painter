...
基于 Maibot 插件的智能多模型图片生成插件，支持文生图和图生图自动识别。兼容OpenAI、豆包、Gemini、魔搭等多种API格式。提供命令式风格转换、模型配置管理、结果缓存等功能。**v3.5.0 重大升级：日常叙事系统让每天的自拍形成连贯故事线，配文多样化支持5种类型，Hybrid混合模式让定时自拍更接近真人习惯。**
